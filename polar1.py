...
def test(test_df, model_path, id2label, label2id):
    ...
    #test_df['polarization'] = 0        
    #test_dataset = PolarizationDataset(test_df['text'].tolist(), test_df['polarization'].tolist(), tokenizer)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lang', '-l', help="Set language to use in training.", type=str, default="")
    ...
    #!!!!!!!!!!!! mix train + dev -> hold-out the validation data
    train = pd.concat([train, val], ignore_index=True)
    train.drop_duplicates(subset="text", inplace=True)
    if args.lang != "":
      train = train[train.lang == args.lang].reset_index(drop=True)
    valid_df = train.groupby(['polarization', 'lang']).apply(lambda x: x.sample(min(100, len(x)), random_state = 0)).sample(frac=1., random_state = 0).reset_index(drop=True)
    train = train[~train.text.isin(valid_df.text)].sample(frac=1., random_state = 0).reset_index(drop=True)
    if args.lang != "":
      train = train.groupby(['polarization']).apply(lambda x: x.sample(max(2000, len(x)), replace=True, random_state = 0)).sample(frac=1., random_state = 0).reset_index(drop=True)
    ...
    add_count = int(len(train) * 0.05)
    train_df = pd.concat([train, anonymized.sample(frac=1., random_state = 0).reset_index(drop=True)[:add_count], lowercased.sample(frac=1., random_state = 1).reset_index(drop=True)[:add_count], uppercased.sample(frac=1., random_state = 2).reset_index(drop=True)[:add_count], homoglyphied.sample(frac=1., random_state = 3).reset_index(drop=True)[:add_count]], ignore_index=True).sample(frac=1., random_state = 0).reset_index(drop=True)
    if args.lang == "":
    ...
    if args.test_only != True:
      fine_tune(train_df, valid_df, f"models/{model}{args.lang}", id2label, label2id, model, args.continue_train)

    # test detector model
    results, predictions, probs = test(test_df, f"models/{model}{args.lang}/best/", id2label, label2id)
    
    #logging.info(results)
    predictions_df = pd.DataFrame({'id': unique_ids, 'polarization': predictions, 'probs': probs[:,1]})
